...
- [x] `WgcWindowCapture` — Windows.Graphics.Capture 方式(既定。OBS の「Windows 10 (1903以降)」相当)。
      PrintWindow では撮れない**管理者権限で動くゲーム**・隠れたウィンドウ・GPU描画に対応。
      `CAPTURE_METHOD` / `--capture-method` で従来方式に切替可(設計は architecture.md)
...
- [x] Live リファクタリング: `Live/Program.cs` のループを `LiveSession` に抽出
      (`LiveEvent` イベント発火 + `LiveSessionOptions` の実行中変更)。CLI の挙動は不変。
      `ManualCommentSource` / `CompositeCommentSource` 追加 + ユニットテスト
- [x] `Studio/` プロジェクト作成(`Microsoft.NET.Sdk.Web`, `Medoz.Studio`。TFM は他プロジェクトに合わせ `net10.0`)。
      `dotnet sln add`。`http://127.0.0.1:5100`(`STUDIO_PORT` で変更可)で待ち受け
- [x] `AppLauncher` — VOICEVOX(`VOICEVOX_EXE_PATH`, `/version` で起動待ち)と
      PuruPuruPNGTuber(`PURUPURU_PATH` = run_local_server.bat、死活は HTTP)の起動・停止・状態表示。
      外部起動済みなら殺さない。`IProcessRunner` 注入でユニットテスト
- [x] REST + SSE API(`/api/status`, `/api/apps/*`, `/api/live/*`, `/api/settings`, `/api/events`)。
      SSE は直近200件リプレイ + 追記配信
- [x] `data/studio.json` — UI で変更した設定の永続化
      (優先順位: デフォルト < persona.json < 環境変数 < studio.json)+ マージのユニットテスト
- [x] `wwwroot/` フロントエンド(素の HTML/CSS/JS、ビルド不要): 会話ログタイムライン・
      コメント注入・起動/配信/設定パネル
- [x] ゲーム実況パネル: `CommentarySessionHost` で GameCommentary の `CommentaryLoop` を
      Studio 内実行(`GET /api/windows` で可視ウィンドウ一覧 → 選択して開始/停止、
      SSE `commentary` イベント、Live セッションとは 409 で相互排他)。
      GameCommentary 参照のため Studio / Studio.Tests の TFM は `net10.0-windows` に変更
      (WGC 対応で `net10.0-windows10.0.19041.0` へ再変更)
- [x] 実況の間隔設定を UI から即時変更可能に: `captureIntervalSec`(CAPTURE_INTERVAL_SEC の env 読み込みも追加)/
      `commentaryTimingMode`(一定間隔 / 発話終了から)/ `commentaryAfterSpeechSec`。
      `CommentaryTiming.NextWaitSec`(純粋関数)+ ユニットテスト。
      合わせて `CommentaryLoop` の発話テキスト出力を `onSpeaking` 注入式にし、Studio 実行時に
      コンソールのテキスト選択(QuickEdit)で実況ループが凍結して発話が止まる不具合を修正
- [x] 配信画面 (OBS ブラウザソース用オーバーレイ): `GET /overlay` +
      `Studio/wwwroot/overlay.{html,css,js}`。PuruPuru の OBS モードを iframe で全面に埋め込み、
      字幕(reply / commentary。感情タグ除去)とコメント一覧を SSE で重ねる。1920x1080 を基準に
      ソースサイズへ scale。`GET /api/overlay/config` でアバター URL を解決。
      `GET /api/events?replay=0`(配信画面に過去ログを出さない)+ ユニットテスト
- [ ] 実機確認: VOICEVOX / APIキーがある環境で「まとめて起動」→ 配信開始 → コメント注入で
      応答・発話まで通す(APIキー無し環境では起動〜設定変更・エラー応答まで確認済み)
- [ ] 実機確認: OBS にブラウザソース (1920x1080, `http://127.0.0.1:5100/overlay`) を追加し、
      アバター・字幕・コメントが1枚で映ること(口パクには PuruPuru の操作画面を開いたままにする)
- 完了条件: `dotnet run --project Studio` → ブラウザから「まとめて起動」→ 配信開始 →
  コメント注入で応答・発話し、会話ログに流れる。設定変更(フリートーク秒数等)が即時反映される

## Phase O: 人格育成(Reflect / 壁打ち)

方針: 人格の核 character.md は自動で書き換えず、AI は提案のみ出す(dry-run 原則)。
提案は `data/<slug>/persona-proposals/<timestamp>.md` に出力し、人間がレビューして手で反映する。

- [x] `Reflect/` — 記憶(配信メモ・ツイート・ブログ)から character.md への追記提案を生成
      (`dotnet run --project Reflect`、`--print` で保存せず表示)。モード指示は `reflect_system.md`
- [x] 壁打ちモード `Reflect -- --interview` — AI がインタビュアーとして質問し、開発者が答える対話で
      人格を生成・補正する。`/done` で会話を提案(追記 or 既存記述の置き換え `replaces`)にまとめて保存、
      `/quit` で保存せず終了。モード指示は `interview_system.md` + ユニットテスト
- [x] `personas/default/` に `reflect_system.md` / `interview_system.md` 追加。
      ぽとふリポジトリ(sibling)にも両ファイルを配置(コミットは手動)
- [x] Claude Code スキル `/persona`(`.claude/skills/persona/SKILL.md`)— Claude Code を
      壁打ち相手にして人格を生成・補正。提案は Reflect と同じ persona-proposals/ 形式で保存し、
      **ユーザーが提案単位で承認したものだけ** character.md に反映(`/persona 提案` で既存提案のレビューも)
- [ ] 実機確認: ぽとふで壁打ち → 提案を character.md に手で反映 → 配信で確認、を1周回す
- 完了条件: 壁打ち1セッションから採用できる粒度の提案 Markdown が出て、
  character.md の git 履歴が人格の成長ログとして残っていく

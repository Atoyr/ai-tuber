...
コメント / ゲーム画面
        │
        ▼
   Claude(人格 = $PERSONA_DIR/character.md)
        │  ← 応答テキスト(+ 感情タグ [fun] など)
        ▼
...
- 入力側は 2 系統: 視聴者コメント(コンソール入力 / YouTube Live / Twitch チャット)と、ゲーム画面のスクリーンショット(Vision)。
...
| 配信本体(Live) | `Live` | コメント → 応答 → 発話。コンソール入力 / YouTube Live / Twitch チャット | 実装済み(YouTube / Twitch の実配信テストは未実施) |
...
# Twitch チャットのコメントを取得(チャンネル名または URL。APIキー不要)
dotnet run --project Live -- --twitch shroud
dotnet run --project Live -- --twitch "https://www.twitch.tv/shroud"
...
| `--twitch <channel\|URL>` | Twitch チャットのコメントを取得。チャンネル名 / `#name` / `twitch.tv/name` URL に対応。**匿名接続で APIキー不要** |
...
**videoId の解決順**: `--youtube` の値 → 位置引数 → 環境変数 `YT_VIDEO_ID`。`--console` も `--youtube` も `--twitch` も無く videoId も無い場合は、使い方を表示して終了します。YouTube モードでは `YOUTUBE_API_KEY` が必須です。

**Twitch モード**: `--twitch <channel|URL>` を指定すると Twitch IRC に**匿名(read-only)接続**してチャットを取得します。OAuth もトークンも不要(**APIキー不要**)です。チャンネル指定は素の名前 / `#name` / `twitch.tv/name` の URL いずれでも可。切断や一時エラー時は数秒後に自動再接続します(`[twitch] 接続エラー: ...(5秒後に再接続)`)。`--console` / `--youtube` / `--twitch` は同時指定した場合 `--twitch` が優先されます。
...
配信・ゲーム実況の音声モードでは、キャラクターが発話の**先頭に感情タグを 1 つ**付けます(`$PERSONA_DIR/character.md` で定義)。タグは読み上げられず、VOICEVOX のスタイル(声色)切り替えに使われます。本文はタグの後に続きます。
...
キャラクターの人格・口調はすべて**ペルソナディレクトリ**(環境変数 `PERSONA_DIR` で指す。未設定時は同梱の `personas/default/`)配下の Markdown が唯一の設定元です。**C# コードを変更する必要はありません**(人格をコードにハードコードしない設計)。現在の人格「ぽとふ」は private リポジトリ [Atoyr/ai-tuber-potofu](https://github.com/Atoyr/ai-tuber-potofu) にあり、sibling に clone して `PERSONA_DIR=../ai-tuber-potofu` で起動します。
...
- **Twitch 実配信での動作テスト(未実施)**
  - 実際の Twitch チャンネルに `dotnet run --project Live -- --twitch <channel|URL>` で接続。
  - 接続時に `[twitch] #<channel> に参加しました` が表示され、視聴者チャットが著者名付きで拾われること。
  - `display-name` が拾えること(空のユーザーは login 名にフォールバック)、本文中の `:` や絵文字が壊れないこと。

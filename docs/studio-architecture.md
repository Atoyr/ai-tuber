...
- プロジェクト: `Studio/`(`Microsoft.NET.Sdk.Web`、`net10.0-windows10.0.19041.0`、名前空間 `Medoz.Studio`)
...
| 種類 | 例 | UI での扱い |
|---|---|---|
| 実行中に反映 | FreetalkAfterSec, CommentBatchSec, フリートークON/OFF, SpeakerId, 感情スタイル, 一時停止, 実況の間隔設定(captureIntervalSec / commentaryTimingMode / commentaryAfterSpeechSec) | 変更即時反映(Live は `LiveSession.UpdateOptions`、実況はループが待ち時間の計算ごとに studio.json を読み直す)+ studio.json に保存 |
...
GET  /api/events             → SSE。LiveEvent と AppLauncher の状態変化を JSON で流す
                               (?replay=0 で接続直後のリプレイを行わない = 配信画面用)
GET  /overlay                → 配信画面 (OBS ブラウザソース用オーバーレイ)
GET  /api/overlay/config     → { avatarUrl, overlayUrl }。avatarUrl は PuruPuru の OBS モード URL
```
...
実況の間隔は UI(設定パネルの「実況」グループ)から即時変更できる:

- `commentaryTimingMode`: `interval`(既定。キャプチャ開始から一定間隔 = CLI と同じ従来挙動)/
  `afterSpeech`(発話が終わってから一定秒数待って次のキャプチャへ)
- `captureIntervalSec`: interval 方式の間隔秒数(既定は CAPTURE_INTERVAL_SEC = 12)
- `commentaryAfterSpeechSec`: afterSpeech 方式の待ち秒数(Studio 固有。既定 5)
- 実況ループは待ち時間の計算ごとに studio.json を読み直すため、変更は次の待ちから反映される
  (待ち時間の計算は `CommentaryTiming.NextWaitSec` に純粋関数として分離しユニットテスト)
- 発話テキストは `CommentaryLoop` の `onSpeaking` コールバック経由で SSE `commentary` イベントに流す。
  **Studio 実行時はコンソールに書かない**(コンソールのテキスト選択 = QuickEdit で1クリックしただけで
  Console.WriteLine がブロックされ、実況ループごと凍結して発話が止まる不具合の対策。CLI は従来どおり表示)
...
- キャプチャ方式は `CAPTURE_METHOD`(既定 `wgc` = Windows.Graphics.Capture)。
  管理者権限で動くゲームも撮れる(詳細は architecture.md「ゲーム実況のキャプチャ方式」)。
  studio.json / UI からの切替は未対応(切り替えたい場合は環境変数)
- この参照のため Studio の TFM は `net10.0-windows10.0.19041.0`
  (ウィンドウ探索が Win32 API 依存 + WGC が Windows 10 1903+ を要求するため)

## 配信画面 (Overlay / OBS ブラウザソース)

OBS に映す配信画面そのものを Studio が配信する。**OBS 側はブラウザソース1個 (1920x1080) で完成する**。

```
OBS ブラウザソース  http://127.0.0.1:5100/overlay   (1920x1080)
┌──────────────────────────────────────────────┐
│ ┌ iframe (全面・透過) ────────────────────┐ │
│ │ PuruPuruPNGTuber OBS モード                │ │ ← アバター (位置・大きさは PuruPuru 側で調整)
│ │ http://127.0.0.1:8223/?mode=obs&transparent=1│ │
│ └────────────────────────────────────────┘ │
│  [コメント一覧]                                 │ ← SSE comments
│                 [ 字 幕 ]                       │ ← SSE reply / commentary
└──────────────────────────────────────────────┘
```

- ファイルは `Studio/wwwroot/overlay.{html,css,js}`。素の HTML/CSS/JS(本体 UI と同じ方針)
- **1920x1080 のステージを組み、`--scale` でブラウザソースのサイズに合わせて縮小する**
  (ソースサイズが違ってもレイアウトが崩れない)。背景は完全透過
- アバターの埋め込み先は `GET /api/overlay/config` の `avatarUrl`(= `PURUPURU_URL` + `?mode=obs&transparent=1`)。
  PuruPuru の OBS モードは**自分ではマイクを触らず**、ローカルサーバの `/api/obs/events`(SSE)から
  口の開き(`voiceRaw`)と顔向きを受け取って描画する。この値を送っているのは PuruPuru の**操作画面**なので、
  **口パクさせるには PuruPuru の操作画面をブラウザで開いたままにしておく必要がある**
  (アバターの見た目・位置・スケールの調整も従来どおり操作画面で行い、「OBSへ反映」でスナップショットを publish する)
- 字幕は `reply` / `commentary` の SSE を受けて表示し、感情タグ(`[joy]` 等)を除去してから出す。
  表示時間は文字数から算出(4〜16秒)。コメントは `comments` の SSE を最大6件・45秒表示
- **SSE は `?replay=0` で購読する**。通常の UI と違い、配信画面では接続直後に過去ログが
  一気に流れてはいけないため(リロードや OBS のソース再作成のたびに古い発話が出るのを防ぐ)
- **配信に映る画面なのでエラーを一切表示しない**。Studio に繋がらなければアバター無しで動き、
  SSE が切れても EventSource の自動再接続に任せて黙って復帰する
- URL クエリで構成を変えられる: `avatar=0` / `comments=0` / `subtitle=0`(各レイヤを消す)、
  `side=left|right`(コメント位置)、`avatarUrl=<URL>`(埋め込み先の上書き)。
  例えば PuruPuru を OBS の別ソースとして置きたい場合は `?avatar=0` で字幕とコメントだけのオーバーレイになる
- 本体 UI の起動パネルから「プレビュー」「URLをコピー」で開ける
...
- **Studio から PuruPuru の口パクを直接駆動する**: PuruPuru のローカルサーバには
  `POST /api/obs/input`(`{targetX, targetY, angleX, angleY, voiceRaw, talking, timestamp}`、
  信頼判定は Host/Origin が 127.0.0.1 / localhost かのみ)がある。`AudioPlayer` に再生中の音量(RMS)を
  取り出すコールバックを足して Studio から voiceRaw を送れば、**マイク(VB-CABLE)も PuruPuru の
  操作画面の開きっぱなしも不要**になり、発話と口の同期も正確になる。感情タグを顔向きに反映することもできる

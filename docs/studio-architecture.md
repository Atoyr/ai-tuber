...
│ │                   ── Process.Start ──► PuruPuruPNGTuber       │
...
- 参照: `Live`(LiveSession / ICommentSource 実装)、`AiTuber.Core`、`Voicevox`、`MultiLLMClient`
...
| PuruPuruPNGTuber | `PURUPURU_EXE_PATH` を `Process.Start` | プロセス生存 + (v2で)ウィンドウタイトル存在確認 | 同上 |

設計原則:

- **すでに起動していたら起動しない**: VOICEVOX は `/version` が応答すれば「起動済み(外部)」、
  PuruPuru はプロセス名一致で検出。この場合 UI の停止ボタンは無効化する
  (**ユーザーが手で起動したアプリを Studio が殺さない**)
- exe パスは環境変数(`VOICEVOX_EXE_PATH` / `PURUPURU_EXE_PATH`)または studio.json。未設定なら
...
event: state
data: {"live":"Running","voicevox":"Running","purupuru":"RunningExternal"}
```
...
- 会話ログは種別で色分け(コメント / 発話 / skip / フリートーク / システム)。自動スクロール+一時停止
...
- GameCommentary / TwitterBot / BlogBot の状態表示と dry-run 実行ボタン(全モードの統合コンソール化)

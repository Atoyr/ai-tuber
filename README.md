...
LLM API キー / VOICEVOX 設定 / 外部アプリ (VOICEVOX の exe・PuruPuruPNGTuber の
run_local_server.bat のパス) / X 認証情報を GUI から一括で設定できる WPF アプリです。
Windows でのみ動作します。
...
`PURUPURU_PATH`(PuruPuru は exe ではなく同梱 `run_local_server.bat` のパス)を設定します
(未設定でも手動起動したアプリを自動検出します)。
